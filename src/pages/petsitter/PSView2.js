...
const PSView2 = () => {
  ...
  //새로 만들어보아요
  const [psInfo, setPSInfo] = useState({}); //펫시터 정보
  const [careInfo, setCareInfo] = useState({}); //돌봄 환경
  const [petInfo, setPetInfo] = useState({}); //반려동물 맨 상단 정보
  const [reviewInfo, setReviewInfo] = useState([]); //리뷰 정보
  //리뷰 배열이 lenght 2미만일 때는 리뷰가 아직 없다고 올리기
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        //펫시터 정보
        let psInfo_array = {};
        psInfo_array.name = res.data[0].userName;
        console.log("psInfo_array", psInfo_array);
        setPSInfo(psInfo_array);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <div id={style.aa}>
      ...
      <div className={style.body}>
        ...
        <div className={style.box}>
          ...
          <div className={style.progress}>
            <ProgressBar completed={10} customLabel="친절도" />
            <ProgressBar completed={10} customLabel="시간약속" />
            <ProgressBar completed={10} customLabel="섬세함" />
          </div>
      ...
      {/* 바텀 */}
      <FooterPS sitterId={params.userId} />
    </div>
  );
};

export default PSView2;
